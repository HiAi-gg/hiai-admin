...
} from '../../modules/integrations/site-access-provisioning.service.js';
import {
  provisionExternalSiteAccessSchema,
  serviceAccessOperationIdSchema,
} from '../validation/integration-site-access.schema.js';
...
const isPlaceholder = (value: string) =>
  /change[-_]?me|placeholder|example|dummy|your[-_]?secret/i.test(value);
  ...
  try {
    ...
    return entries.map((entry) => {
      ...
      if (
        ...
        !scopes.includes('site-access:provision')
      )
        throw new Error('Invalid SERVICE_INTEGRATIONS_JSON configuration');
      ids.add(id);
...
const verifyServiceToken = (request: Request, operationId: string): ServiceClaims | null => {
  ...
  if (
    ...
    return null;
  const config = getServiceConfig().find(
    (entry) => entry.issuer === claims.iss && entry.audience === claims.aud && entry.secret,
  );
  if (
    !config ||
    !Array.isArray(claims.scope) ||
    !claims.scope.includes('site-access:provision') ||
    !config.scopes.includes('site-access:provision')
  )
    return null;
...
export const integrationSiteAccessRoutes = new Elysia({ prefix: '/api/integrations' })
  .post('/site-access', async ({ request, body, set }) => {
    ...
    try {
      return await provisionExternalSiteAccess(parsed.data, claims.jti);
    } catch (error) {
      const code = error instanceof Error ? error.message : 'PROVISIONING_FAILED';
      set.status =
        code === 'OPERATION_PAYLOAD_MISMATCH' || code === 'OPERATION_TOKEN_MISMATCH' ? 409 : 422;
      return { error: code };
    }
  })
  .delete(
    '/site-access/:operationId',
    async ({ request, params, set }) => {
      ...
      try {
        await compensateExternalSiteAccess(params.operationId, claims.jti);

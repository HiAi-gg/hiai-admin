...
const operationResult = (value: unknown): SiteAccessProvisioningResult =>
  value as SiteAccessProvisioningResult;
...
export async function provisionExternalSiteAccess(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (existing[0]) {
      ...
      if (existing[0].status === 'succeeded' && existing[0].response)
        return operationResult(existing[0].response);
    } else {
    ...
    const [owner] = await tx
      ...
      .where(
        and(
          eq(users.id, request.owner.platformUserId),
          eq(users.email, request.owner.email.toLowerCase()),
        ),
      )
      .limit(1);
    ...
    if (!tenant) {
      const [existingTenant] = await tx
        .select()
        .from(tenants)
        .where(eq(tenants.slug, request.tenant.slug))
        .limit(1);
      if (
        !existingTenant ||
        existingTenant.name !== request.tenant.name ||
        (existingTenant.settings as { externalId?: string } | null)?.externalId !==
          request.tenant.externalId
      )
        throw new Error('TENANT_CONFLICT');
      throw new Error('TENANT_ALREADY_EXISTS');
    }

    await tx
      .insert(userTenantAccess)
      .values({ userId: owner.id, tenantId: tenant.id, role: 'owner', permissions: ['all'] });
    ...
    await tx
      ...
      .where(eq(siteAdapters.id, adapter.id));
    await tx
      .update(tenants)
      .set({ status: 'active', updatedAt: new Date() })
      .where(eq(tenants.id, tenant.id));
    await tx.insert(auditLogs).values({
      actorId: `service:${tokenJti}`,
      actorEmail: 'service-integration',
      action: 'site_access:provision',
      resource: 'site_access',
      resourceId: request.operationId,
      newValue: { tenantId: tenant.id, siteAdapterId: adapter.id },
    });
    return {
      operationId: request.operationId,
      tenantId: tenant.id,
...
export async function compensateExternalSiteAccess(
  operationId: string,
  tokenJti: string,
): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    const [operation] = await tx
      .select()
      .from(integrationOperations)
      .where(eq(integrationOperations.operationId, operationId))
      .limit(1);
    ...
    await tx
      ...
      .where(eq(siteAdapters.id, operation.siteAdapterId));
    await tx
      .update(tenants)
      .set({ status: 'suspended', updatedAt: new Date() })
      .where(eq(tenants.id, operation.tenantId));
    await tx
      .update(integrationOperations)
      .set({ status: 'compensated', updatedAt: new Date(), completedAt: new Date() })
      .where(eq(integrationOperations.operationId, operationId));
    await tx.insert(auditLogs).values({
      actorId: `service:${tokenJti}`,
      actorEmail: 'service-integration',
      action: 'site_access:compensate',
      resource: 'site_access',
